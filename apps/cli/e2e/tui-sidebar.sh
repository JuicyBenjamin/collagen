...
( while [ ! -f "$OUT/tui.go" ]; do sleep 1; done; sleep 3; mark M0_initial; printf '\033[D'; sleep 2; mark M1_rail; printf '\033[A'; sleep 1; mark M2_up; printf '\r'; sleep 4; mark M3_switched
  ...
  COLLAGEN_DEV=1 COLLAGEN_LOG="$OUT/tui.log" script -F -q "$PTY" bash -c 'stty rows 40 cols 120; node --experimental-ffi --import tsx src/index.tsx --profile alice' > /dev/null 2>&1 &
sleep 12
...
sleep 4
expect "alice's room shows bob online and one unread" "$(call $A "$SA" list-rooms '{}')" "dev room,2,1,true"
touch "$OUT/tui.go"
