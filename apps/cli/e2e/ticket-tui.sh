...
( while [ ! -f "$OUT/ticket-tui.go" ]; do sleep 1; done; sleep 2
  ...
  COLLAGEN_DEV=1 COLLAGEN_LOG="$LOG" script -F -q "$PTY" bash -c 'stty rows ${ROWS:-45} cols 120; node --experimental-ffi --import tsx src/index.tsx --profile alice' > /dev/null 2>&1 &
sleep 12
