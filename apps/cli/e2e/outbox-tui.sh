...
expect "no counts on the tabs: a log's size is not news" "$(echo "$TEXT" | grep -cE 'outbox \([0-9]+\)|messages \([0-9]+\)')" "^0$"
...
if [ "$HAVE_PYTE" = yes ]; then
  ...
  expect "…the header says how far through the text you are, and nothing else" "$(render M3b_end)" "outbox 23/23"
