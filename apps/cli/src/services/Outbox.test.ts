...
/** A Dispatch that records what it was asked to write instead of touching a room. */
const recording = () => {
  const wrote: Array<{ roomId: string; out: Outgoing }> = [];
  const layer = Layer.succeed(Dispatch, {
    perform: (roomId: string, out: Outgoing) => Effect.sync(() => (wrote.push({ roomId, out }), `wrote ${out.kind}`)),
  } as const);
  return { wrote, layer };
};

const run = <A>(body: (o: Outbox["Service"]) => Effect.Effect<A>, sent?: LocalState["sent"]) => {
  const d = recording();
  return Effect.runPromise(
    Effect.gen(function* () {
      return yield* body(yield* Outbox);
    }).pipe(Effect.provide(Outbox.layer.pipe(Layer.provide(stateWith(sent)), Layer.provide(d.layer)))),
...
describe("the outbox — what went out", () => {
  it("sends at once and tells the agent what happened, not what might", async () => {
    ...
    expect(value.outcome).toBe("wrote message");
