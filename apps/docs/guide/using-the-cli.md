...
- **overview tab** — who's here (with their AI and its auth badge; `○ (away)` for peers
  looking at another room), every ticket in the room that is not over — whoever made it
  and whoever it is for; a finished one has left the list and stays on the log — and the
  projects section. A ticket row says whose
